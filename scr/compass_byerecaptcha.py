...
def solveByImage():
    ...
    while True:
        counter_total += 1
        if counter_total >= 10:
            raise Exception("try too many times, byerecaptcha can not handle")
        if counter_16 >= 4:
            raise Exception("too many 4*4 image, byerecaptcha can not handle")
...
def solveRecaptcha(browser, server='', invisible=False):
    ...
    if not invisible:
        ...
        try:
            result = getRecaptchaResponse()
            if result is True:
                print('Passed without picture click ！！！congrats')
                return result
        except:
            pass
    while True:
